...
class RedWineData(DataLoader):
    def __init__(self, path='data/winequality-red.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def _preprocess_data(self):
        self._data.loc[self._data['quality'] < 5.5, 'quality'] = 0
        self._data.loc[self._data['quality'] > 5.5, 'quality'] = 1

    def pre_training_adjustment(self, train_features, train_classes):
        return train_features, train_classes

class AdultData(DataLoader):
    def __init__(self, path='data/adult.data.txt', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def _preprocess_data(self):
        self._data.columns = ['age','employer','fnlwt','edu','edu_num','marital','occupation','relationship','race','sex','cap_gain','cap_loss','hrs','country','income']
        ...
        self._data = self._data.replace(to_replace={'country':replacements,
...
if __name__ == '__main__':
    ...
    rw_data = RedWineData(verbose=True)
    rw_data.load_and_process()

    a_data = AdultData(verbose=True)
    a_data.load_and_process()
